...
MMLIST *mmlist_init(char *file)
{
    ...
    if(file && (mlist = (MMLIST *)calloc(1, sizeof(MMLIST))))
    {
       ...
       {
           size = mlist->msize = (off_t)sizeof(MMSTATE) + (off_t)sizeof(MMSLOT) * (off_t)MM_NODES_MAX;
           mlist->state = (MMSTATE*)mmap(NULL, size, PROT_READ|PROT_WRITE, MAP_SHARED, mlist->fd, 0);
           mlist->slots = (MMSLOT *)((char *)mlist->state + sizeof(MMSTATE));
           fstat(mlist->fd, &st);
...
int mmlist_slot_new(MMLIST *mlist)
{
    ...
    {
        size = mlist->size + (off_t)sizeof(MMSLOT);
        ftruncate(mlist->fd, size);
        memset(((char *)mlist->state+mlist->size), 0, sizeof(MMSLOT));
        ret = (mlist->size - (off_t)sizeof(MMSTATE)) / (off_t)sizeof(MMSLOT);
        mlist->size = size;
    }
    return ret;
}
#define VX(xxx) mlist->state->roots[xxx]
#define PSLOT(xxx) mlist->slots[xxx] 
#define VSLOT(xxx) mlist->slots[(VX(xxx))] 
int mmlist_insert(MMLIST *mlist, int no, int32_t key)
{
    ...
    if(mlist && mlist->state && (vnodes = mlist->vmap))
    {
        if((n = mlist->state->count) > 0)
        {
            max = n - 1;
            min = 0;
            if(key <= VSLOT(min).max) k = min;
            else if(key >= VSLOT(max).min) k = max;
            else
            {
                while(max > min)
                {
                    ...
                    if(key >=  VSLOT(x).min && key <= VSLOT(x+1).min)
                    {
                    ...
                    else if(key > VSLOT(x).max) min = x;
                    else max = x;
                }
            }
        }
        /* 未满的slot 直接插入 */
        if(k >= 0 && k < n && VSLOT(k).count < MM_KV_NUM)
        {
            x = VSLOT(k).count++;
            ...
            {
                memcpy(&(kvs[x]), &(kvs[x-1]), sizeof(MMKV));
                /*
            ...
            kvs[x].key = key;
            kvs[x].val = no;
            if(key < VSLOT(k).min) VSLOT(k).min = key;
            if(key > VSLOT(k).max) VSLOT(k).max = key;
        }
        else
        {
                    ...
                    //&& key >= mlist->slots[k].min && key < mlist->slots[k].max)
            {
                ...
                kvs = VSLOT(k).kvs + MM_KV2_NUM;
                kv = PSLOT(nodeid).kvs;
                num = MM_KV2_NUM;
                for(i = 0; i < MM_KV2_NUM; i++) 
                {
                    if(key >= kvs->key && key < kvs[i].key && num == MM_KV2_NUM) 
                    {
                        //fprintf(stdout, "%s::%d k:%d no:%d count:%d min:%d max:%d key:%d OK\n", __FILE__, __LINE__, k, no, mlist->slots[k].count, mlist->slots[k].min, mlist->slots[k].max, key);
                        kv->key = key;
                        kv->val = no;
                ...
                if(num == MM_KV2_NUM && key >= kvs[i-1].key)
                {
                    //fprintf(stdout, "%s::%d k:%d no:%d count:%d min:%d max:%d key:%d OK\n", __FILE__, __LINE__, k, no, mlist->slots[k].count, mlist->slots[k].min, mlist->slots[k].max, key);
                    kv->key = key;
                    kv->val = no;
                ...
                if(num == MM_KV2_NUM)
                {
                    ...
                    kvs[i].key = key;
                    kvs[i].val = no;
                    VSLOT(k).min = kvs[0].key;
                    VSLOT(k).max = kvs[MM_KV2_NUM].key;
                    VSLOT(k).count = MM_KV2_NUM + 1;
                    //fprintf(stdout, "%s::%d k:%d no:%d count:%d min:%d max:%d key:%d OK\n", __FILE__, __LINE__, k, no, mlist->slots[k].count, mlist->slots[k].min, mlist->slots[k].max, key);
                    //pos = k;
                ...
                else
                {
                    ...
                    kvs = VSLOT(k).kvs;
                    VSLOT(k).min = kvs[0].key;
                    VSLOT(k).max = kvs[MM_KV2_NUM-1].key;
                    VSLOT(k).count = MM_KV2_NUM;
                    //fprintf(stdout, "%s:%d k:%d no:%d count:%d min:%d max:%d key:%d OK\n", __FILE__, __LINE__, k, no, mlist->slots[k].count, mlist->slots[k].min, mlist->slots[k].max, key);
                }
                pos = k+1;
                k = mlist->state->count++; 
                ...
                {
                    VX(k) = VX(k-1);
                    //mlist->state->roots[k] = mlist->state->roots[k-1];
                    //memcpy(&(mlist->slots), &(mlist->slots[k-1]), sizeof(MMSLOT));
                    /*
            ...
            //kv = mlist->map + nodeid;
            kv = PSLOT(nodeid).kvs;
            PSLOT(nodeid).min = kv[0].key;
            PSLOT(nodeid).max = kv[num-1].key;
            PSLOT(nodeid).count = num;
...
int main()
{
    ...
    if((mlist = mmlist_init("/tmp/1.idx")))
    {
        ...
        {
            fprintf(stdout, "%d:{min:%d max:%d}(%d)\n", i, VSLOT(i).min, VSLOT(i).max, VSLOT(i).count);
        }
        mmlist_close(mlist);
    }
}
