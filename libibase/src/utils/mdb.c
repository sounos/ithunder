...
#endif
static MAPTAB mdb_xblock_list[] = {{4096,1024},{8192,1024},{16384,1024},{32768,1024},{65536,1024},{131072,1024},{262144,1024},{524288,512},{1048576,256},{2097152,64},{4194304,32},{8388608,16},{16777216,8},{33554432,4},{67108864,2}};
...
MDB *mdb_init(char *dbdir, int mode)
{
    ...
    if(dbdir && (db = (MDB *)xmm_mnew(sizeof(MDB))))
    {
                ...
                && fstat(db->lnkio.fd, &st) == 0)
        {
            ...
            if(st.st_size == 0)
            {
                db->lnkio.end = db->lnkio.size = sizeof(XLNK) * MDB_LNK_MAX * 2;
...
int mdb_push_block(MDB *db, int index, int blockid, int block_size)
{
            ...
            && db->status == 0 && index >= 0 && index < MDB_MFILE_MAX)
    {
        ...
        if(x >= MDB_LNK_MAX && (i = index) >= 0 && i < MDB_MFILE_MAX 
...
int mdb_pop_block(MDB *db, int blocks_count, XLNK *lnk)
{
    ...
    {
        RWLOCK_WRLOCK(db->mutex_lnk);
        if(x < MDB_LNK_MAX && (links = (XLNK *)(db->lnkio.map)) && links[x].count > 0 
                ...
                && db->dbsio[index].fd > 0) 
        {
            ...
            if(--(links[x].count) > 0)
            {
                ...
                {
                    //if(pread(db->dbsio[index].fd, &link, sizeof(XLNK), (off_t)links[x].blockid*(off_t)MDB_BASE_SIZE) >0)
                    if(mdb_pread(db, index, &link, sizeof(XLNK), (off_t)(links[x].blockid)*(off_t)MDB_BASE_SIZE) >0)
                    {
...
void mdb_destroy(MDB *db)
{
    ...
    if(db)
    {
        ...
        if((db->dbsio[0].fd = open(path, O_CREAT|O_RDWR, 0644)) > 0)
        {
            ...
            db->dbsio[0].end = db->dbsio[0].size = MDB_MFILE_SIZE;
