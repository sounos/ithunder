#!/bin/bash
mkdir -p backup ithunder; datetime=`date +%Y%m%d%H%M%S`;tar -zcf backup/all-${datetime}.tar.gz rpms debuginfo/ src srpms/ *.sh; tar -zcf backup/rpms-${datetime}.tar.gz rpms debuginfo/; tar -zcf backup/source-${datetime}.tar.gz src srpms *.sh; 
#sodo rpms
...
#ithunder rpms
tar -zcf ithunder/rpms-${datetime}.tar.gz rpms/libevbase* rpms/libsbase*  rpms/libscws* rpms/libibase* rpms/libchardet* rpms/hibase* rpms/libdbase* rpms/libmtask* rpms/qmtask* rpms/hidbase* rpms/ithunder* debuginfo/libevbase* debuginfo/libsbase*  debuginfo/libscws* debuginfo/libibase* debuginfo/libchardet* debuginfo/hibase* debuginfo/qmtask* debuginfo/hidbase* debuginfo/ithunder* ; 
#ithunder srpms
