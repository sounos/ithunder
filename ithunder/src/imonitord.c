    ...
    if(conn && buffer->ndata > 0 && buffer->data)
    {
        ...
        do
        {
            if((n = evdns_parse_reply(s, left, &hostent)) > 0)
            {
                s += n;
                left -= n;
                DEBUG_LOGGER(adnsd_logger, "name:%s left:%d naddrs:%d", 
                ...
                if(hostent.naddrs > 0)
                {
                    ...
                    ip = hostent.addrs[0];
                    p = (unsigned char *)&ip;
                    DEBUG_LOGGER(adnsd_logger, "Got host[%s]'s ip[%d.%d.%d.%d] from %s:%d", 
...
int adnsd_packet_handler(CONN *conn, CB_DATA *packet)
{
    ...
    if(conn)
    {
        ...
        conn->over(conn);
        xtask_set_dns_status(xtask, tid, NULL, DNS_STATUS_ERR);
    }
    return -1;
}
...
int adnsd_ok_handler(CONN *conn)
{
    ...
    if(conn)
    {
        ...
        else
        {
            ...
            return conn->set_timeout(conn, conn->timeout + adns_trans_timeout);
        }
    }
    return -1;
}
...
int http_bind_proxy(CONN *conn, char *host, int port) 
{
    ...
    if(conn && host && port > 0)
    {
        if((bitip = xtask_get_host_ip(xtask, host)) != -1) 
        {
...
int httpd_request_handler(CONN *conn, HTTP_REQ *http_req)
{
    ...
    char buf[HTTP_LINE_MAX], *p = NULL, *end = NULL, *name = NULL, *host = NULL, 
         ...
         *pp = NULL, *user = NULL, *passwd = NULL, format[URL_LEN_MAX],
         block[HTTP_BUF_SIZE];
        ...
        switch(op)
        {
            ...
            case E_OP_URLNODE_ADD:
                ...
                else
                {
                    ...
                    //fprintf(stdout, "%s::%d parentid:%d nodeid:%d url:%s flag:%d urlid:%d\r\n", __FILE__, __LINE__, parentid, nodeid, url, flag, urlid);
                }
                if(parentid > 0)
                    goto urlnodes_list;
                else
        ...
        if((chunk = conn->newchunk(conn, XT_TASK_SIZE))) 
        {
            ...
            if((req->cmd & XT_REQ_DOWNLOAD)) 
            {
                ...
                if((n = xtask_new_download(xtask, req->flag, &taskid, block, n)) > 0)
                {
                    ...
                    {
                        WARN_LOGGER(logger, "NEW_DOWNLOAD_TASK:%d ndata:%d", conn->xids[0], n);
                        chunk = NULL;
                        return ret = 0;
                    }
                }
            ...
            else if((req->cmd & XT_REQ_EXTRACT)) 
            {
                ...
                if((n = xtask_new_extract(xtask, &taskid, block, n)) > 0)
                {
                    ...
                    if(conn->send_chunk(conn, chunk, n + sizeof(XTHEAD)) == 0)
                    {
                        WARN_LOGGER(logger, "NEW_EXTRACT_TASK:%d ndata:%d", conn->xids[0], n);
                        chunk = NULL;
                        return ret = 0;
                    }
                }
...
int monitord_data_handler(CONN *conn, CB_DATA *packet, CB_DATA *cache, CB_DATA *chunk)
{
    ...
    if(conn && packet && (head = (XTHEAD *)(packet->data)))
    {
        ...
        {
            WARN_LOGGER(logger, "OVER_DOWNLOAD_TASK:%d ndata:%d", conn->xids[0], chunk->ndata);
            ret = xtask_over_download(xtask, conn->xids[1], conn->xids[0], chunk->data, chunk->ndata);
        }
        else if(head->cmd & XT_REQ_EXTRACT)
        {
            WARN_LOGGER(logger, "OVER_EXTRACT_TASK:%d ndata:%d", conn->xids[0], chunk->ndata);
