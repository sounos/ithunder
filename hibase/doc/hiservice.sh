...
genservice()
{
    ...
    popd;
    /bin/cp -f /etc/rc.d/init.d/hidocd /etc/rc.d/init.d/$docd
    /bin/cp -f /etc/rc.d/init.d/hindexd /etc/rc.d/init.d/$indexd
    /bin/cp -f /etc/rc.d/init.d/hiqparserd /etc/rc.d/init.d/$parserd
    /bin/cp -f /etc/rc.d/init.d/hiqdocd /etc/rc.d/init.d/$qdocd
    /bin/cp -f /etc/rc.d/init.d/himasterd /etc/rc.d/init.d/$masterd
...
#qdoc
    ...
    perl -i -p -e "s/hidoc/$doc/g" /etc/rc.d/init.d/$docd /etc/"${docd}.ini"
    perl -i -p -e "s/HIDOC/`echo $doc|tr [a-z] [A-Z]`/g" /etc/rc.d/init.d/$docd
    perl -i -p -e "s/hindex/$index/g" /etc/rc.d/init.d/$indexd /etc/"${indexd}.ini"
    perl -i -p -e "s/HINDEX/`echo $index|tr [a-z] [A-Z]`/g" /etc/rc.d/init.d/$indexd
    perl -i -p -e "s/hiqparser/$parser/g" /etc/rc.d/init.d/$parserd /etc/"${parserd}.ini"
    perl -i -p -e "s/HIQPARSER/`echo $parser|tr [a-z] [A-Z]`/g" /etc/rc.d/init.d/$parserd
    perl -i -p -e "s/hiqdoc/$qdoc/g" /etc/rc.d/init.d/$qdocd /etc/"${qdocd}.ini"
    perl -i -p -e "s/HIQDOC/`echo $qdoc|tr [a-z] [A-Z]`/g" /etc/rc.d/init.d/$qdocd
    perl -i -p -e "s/himaster/$master/g" /etc/rc.d/init.d/$masterd /etc/"${masterd}.ini"
    perl -i -p -e "s/HIMASTER/`echo $master|tr [a-z] [A-Z]`/g" /etc/rc.d/init.d/$masterd
