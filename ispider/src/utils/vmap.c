...
{                                                                               \
    ...
    {                                                                           \
        ...
        {                                                                       \
            ...
            if(VMP(x)->rold == 0) VMP(x)->state->rleft--;                       \
...
unsigned int vmap_next(void *x, int rootid, unsigned int tnodeid, int *key, int *data)
{
    ...
    if(x && tnodeid > 0)
    {
        ...
        {
            id = tnodeid;
            //int rid = VMP(x)->roots[rootid].rootid;
            //fprintf(stdout, "%s::%d rootid:%d{parent:%d left:%d right:%d} node:%d{parent:%d left:%d right:%d)\n", __FILE__, __LINE__, rid, VMP(x)->map[rid].parent, VMP(x)->map[rid].left, VMP(x)->map[rid].right, id, VMP(x)->map[id].parent, VMP(x)->map[id].left, VMP(x)->map[id].right);
