...
int dmap_remove(DMAP *dmap, u32_t no)
{
            ...
            && (nodeid = dmap->vmap[no].off) >= 0)
    {
        ...
        {
            i = slotid;
            dmap->state->qleft[(dmap->state->nleft++)] = nodeid;
            dmap->roots[rootid] = -1;
            while(i < dmap->state->count)
            {
        ...
        else
        {
            ...
            if(n > 0) dmap->slots[slotid].max = kvs[n - 1].key;
        }
        dmap->vmap[no].off = -1;
    }
    return ret;
...
int main()
{
    ...
    if((dmap = dmap_init("/tmp/1.idx")))
    {
        ...
        {
            val = inputs[i];
            stat2[val] = dmap_in(dmap, val, res);
        }
        TIMER_SAMPLE(timer);
